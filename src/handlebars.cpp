...
#include <boost/handlebars/handlebars.hpp>
    ...
    HandlebarsOptions const& opt)
{
    ...
    BOOST_HANDLEBARS_TRY(checkPath(path, state));
    ...
    lookupLevel(dom::Value level) const {
        if (level.isString()) {
            ...
            auto toLower = [](char c) {
                ...
                return static_cast<char>(c + diff * isLower);
            };
...
{
    BOOST_HANDLEBARS_ASSERT(tagStr.size() >= 4);
    Handlebars::Tag t;
    t.escaped = tagStr.front() == '\\';
    BOOST_HANDLEBARS_ASSERT(tagStr[0 + t.escaped] == '{');
    BOOST_HANDLEBARS_ASSERT(tagStr[1 + t.escaped] == '{');
    BOOST_HANDLEBARS_ASSERT(tagStr[tagStr.size() - 1] == '}');
    BOOST_HANDLEBARS_ASSERT(tagStr[tagStr.size() - 2] == '}');
    t.buffer = tagStr;
    ...
    if (checkStandalone)
    {
        BOOST_HANDLEBARS_ASSERT(t.buffer.data() >= context.data());
        BOOST_HANDLEBARS_ASSERT(t.buffer.data() + t.buffer.size() <= context.data() + context.size());
    ...
    detail::RenderState& state) const
{
    while (!state.templateText.empty())
    {
        ...
        // Render tag
        // ==============================================================
        BOOST_HANDLEBARS_TRY(renderTag(tag, out, context, opt, state));
    ...
    bool evalLiterals) const
{
    ...
    {
        BOOST_HANDLEBARS_TRY(checkPath(expression, state));
        ...
        BOOST_HANDLEBARS_TRY(auto r, lookupPropertyImpl(data, expression, state, opt));
        auto [res, found] = r;
    ...
    if (expression.starts_with("..")) {
        ...
        BOOST_HANDLEBARS_TRY(auto r, lookupPropertyImpl(parentCtx, expression, state, noStrict));
        auto [res, found] = r;
        return Res{res, found, false};
    }
    // ==============================================================
    ...
    if (isPathedValue)
    {
        BOOST_HANDLEBARS_TRY(std::tie(r, defined), lookupPropertyImpl(context, expression, state, noStrict));
        if (defined) {
    ...
    BOOST_HANDLEBARS_TRY(std::tie(r, defined), lookupPropertyImpl(context, expression, state, strictOpt));
    if (defined) {
        return Res{r, defined, false};
    }
    ...
    if (opt.compat)
    {
        ...
        {
            BOOST_HANDLEBARS_TRY(std::tie(r, defined), lookupPropertyImpl(parentContext, expression, state, noStrict));
            if (defined)
            {
                return Res{r, defined, false};
            }
    ...
    -> std::pair<dom::Function, bool>
{
    ...
    BOOST_HANDLEBARS_ASSERT(it != helpers_.end());
    ...
    detail::RenderState& state) const
{
    ...
    if (it != helpers_.end()) {
        ...
        HandlebarsOptions noStrict = opt;
        noStrict.strict = false;
        BOOST_HANDLEBARS_TRY(setupArgs(tag.arguments, context, state, args, cb, noStrict));
    ...
    BOOST_HANDLEBARS_TRY(auto resV, evalExpr(context, helper_expr, state, opt, false));
    if (resV.found)
    {
    ...
    HandlebarsOptions const& opt) const
{
    ...
    while (findExpr(expr, expression))
    {
        ...
        if (isPositional)
        {
            ...
            // Positional argument
            // ==========================================
            BOOST_HANDLEBARS_TRY(auto res, evalExpr(context, expr, state, opt, true));
        ...
        else
        {
            ...
            // Named argument
            // ==========================================
            BOOST_HANDLEBARS_TRY(auto res, evalExpr(context, v, state, opt, true));
    ...
    detail::RenderState& state) const
{
    ...
    if (tag.type2 == '#')
    {
        ...
        BOOST_HANDLEBARS_TRY(this->try_render_to_impl(dumb, context, opt, state));
        state.templateText = templateText;
    ...
    if (!tag.arguments.empty())
    {
        ...
        while (findExpr(expr, tagContent))
        {
            ...
            if (contextKey != ".")
            {
                BOOST_HANDLEBARS_TRY(res, evalExpr(context, contextKey, state, opt, true));
            }
            else
            {
                res.value = context;
    ...
    // Render partial
    // ==========================================
    BOOST_HANDLEBARS_TRY(this->try_render_to_impl(out, partialCtx, opt, state));
...
renderBlock(
    ...
    std::string_view inverseBlock;
    Tag inverseTag;
    BOOST_HANDLEBARS_TRY(parseBlock(
    ...
    {
        BOOST_HANDLEBARS_TRY(auto res, evalExpr(context, tag.helper, state, opt, false));
            ...
            dom::Value const& options) -> Expected<void, HandlebarsError>
    {
        ...
        // Render
        // ==========================================
        BOOST_HANDLEBARS_TRY(try_render_to_impl(out, newContext, opt, state));
            ...
            dom::Value const& options) -> Expected<void, HandlebarsError>
    {
        ...
        else
        {
            ...
            // inverse tags.
            BOOST_HANDLEBARS_TRY(renderBlock(blockName, inverseTag, out, newContext, opt, state, true));
        }
...
if_fn(dom::Array const& arguments)
{
    ...
    {
        BOOST_HANDLEBARS_TRY(conditional, conditional.getFunction().try_invoke(context));
    }

    if ((!options.lookup("hash.includeZero") && !conditional) || isEmpty(conditional)) {
        BOOST_HANDLEBARS_TRY(options.get("write_inverse").getFunction().try_invoke(context));
    }
    else
    {
        BOOST_HANDLEBARS_TRY(options.get("write").getFunction().try_invoke(context));
    }
    return {};
}
...
with_fn(dom::Array const& arguments)
{
    ...
    if (context.isFunction()) {
        BOOST_HANDLEBARS_TRY(context, context.getFunction().try_invoke(options.get("context")));
    }
    ...
    if (!isEmpty(context))
    {
        ...
        BOOST_HANDLEBARS_TRY(options.get("write").getFunction().try_invoke(context, cbOpt));
    }
    else
    {
        BOOST_HANDLEBARS_TRY(options.get("write_inverse").getFunction().try_invoke(options.get("context")));
    }
    return {};
}
...
each_fn(dom::Value context, dom::Value const& options)
{
    ...
    if (isJSObject)
    {
        if (context.isArray())
        {
            ...
            for (; i < n; ++i)
            {
                ...
                BOOST_HANDLEBARS_TRY(execIteration(i, static_cast<std::int64_t>(i), isLast));
            }
        }
        else if (context.isObject())
        {
                ...
                dom::String const& key, dom::Value const& /* value */ ) -> Expected<void>
            {
                ...
                {
                    BOOST_HANDLEBARS_TRY(execIteration(priorKey, i - 1, false));
                }
                priorKey = key;
                ++i;
                return {};
            ...
            if (!priorKey.isUndefined())
            {
                BOOST_HANDLEBARS_TRY(execIteration(priorKey, i - 1, true));
            }
        }
    }
    ...
    if (i == 0)
    {
        BOOST_HANDLEBARS_TRY(inverse.getFunction().try_invoke(options.get("context")));
    }
    return {};
}
...
log_fn(dom::Array const& arguments)
{
    ...
    args.set(0, level);
    BOOST_HANDLEBARS_TRY(options.get("log").getFunction().call(args));
    return {};
}
    ...
    dom::Value const& context, dom::Value options)
{
    if (context == true)
    {
        BOOST_HANDLEBARS_TRY(options.get("write").getFunction().try_invoke(options.get("context")));
    }
    else if (
        context == false ||
        context.isNull() ||
    ...
    else if (context.isArray())
    {
        if (!context.empty())
        {
            ...
            BOOST_HANDLEBARS_TRY(each_fn(context, options));
        }
        else
        {
            BOOST_HANDLEBARS_TRY(options.get("write_inverse").getFunction().try_invoke(options.get("context")));
    ...
    else
    {
        ...
        BOOST_HANDLEBARS_TRY(options.get("write").getFunction().try_invoke(context, fnOpt));
    }
    return {};
}
