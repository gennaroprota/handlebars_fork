...
#ifndef BOOST_HANDLEBARS_SUPPORT_EXPECTED_HPP
#define BOOST_HANDLEBARS_SUPPORT_EXPECTED_HPP

#include <boost/handlebars/config.hpp>
#include <boost/handlebars/support/assert.hpp>
#include <boost/handlebars/support/error.hpp>
#include <boost/handlebars/support/source_location.hpp>
...
#ifndef BOOST_HANDLEBARS_TRY
#    define BOOST_HANDLEBARS_MERGE_(a, b) a##b
#    define BOOST_HANDLEBARS_LABEL_(a)    BOOST_HANDLEBARS_MERGE_(expected_result_, a)
#    define BOOST_HANDLEBARS_UNIQUE_NAME  BOOST_HANDLEBARS_LABEL_(__LINE__)

/// Try to retrive expected-like type
#    define BOOST_HANDLEBARS_TRY_VOID(expr)                          \
        auto BOOST_HANDLEBARS_UNIQUE_NAME = expr;                    \
        if (detail::failed(BOOST_HANDLEBARS_UNIQUE_NAME)) {          \
            return Unexpected(detail::error(BOOST_HANDLEBARS_UNIQUE_NAME)); \
        }                                                 \
        void(0)
#    define BOOST_HANDLEBARS_TRY_VAR(var, expr)                      \
        auto BOOST_HANDLEBARS_UNIQUE_NAME = expr;                    \
        if (detail::failed(BOOST_HANDLEBARS_UNIQUE_NAME)) {          \
            return Unexpected(detail::error(BOOST_HANDLEBARS_UNIQUE_NAME)); \
        }                                                  \
        var = *std::move(BOOST_HANDLEBARS_UNIQUE_NAME)
#    define BOOST_HANDLEBARS_TRY_MSG(var, expr, msg)                 \
        auto BOOST_HANDLEBARS_UNIQUE_NAME = expr;                    \
        if (detail::failed(BOOST_HANDLEBARS_UNIQUE_NAME)) {          \
            return Unexpected(Error(msg));                 \
        }                                                  \
        var = *std::move(BOOST_HANDLEBARS_UNIQUE_NAME)
#    define BOOST_HANDLEBARS_TRY_GET_MACRO(_1, _2, _3, NAME, ...) NAME
#    define BOOST_HANDLEBARS_TRY(...) \
        BOOST_HANDLEBARS_TRY_GET_MACRO(__VA_ARGS__, BOOST_HANDLEBARS_TRY_MSG, BOOST_HANDLEBARS_TRY_VAR, BOOST_HANDLEBARS_TRY_VOID)(__VA_ARGS__)

/// Check existing expected-like type
#    define BOOST_HANDLEBARS_CHECK_VOID(var)                         \
        if (detail::failed(var)) {                         \
            return Unexpected(detail::error(var));         \
        }                                                  \
        void(0)
#    define BOOST_HANDLEBARS_CHECK_MSG(var, msg)                     \
        if (detail::failed(var)) {                         \
            return Unexpected(Error(msg));                 \
        }                                                  \
        void(0)
#    define BOOST_HANDLEBARS_CHECK_GET_MACRO(_1, _2, NAME, ...) NAME
#    define BOOST_HANDLEBARS_CHECK(...) \
        BOOST_HANDLEBARS_CHECK_GET_MACRO(__VA_ARGS__, BOOST_HANDLEBARS_CHECK_MSG, BOOST_HANDLEBARS_CHECK_VOID)(__VA_ARGS__)

/// Check existing expected-like type and return custom value otherwise
#    define BOOST_HANDLEBARS_CHECK_OR_VOID(var)                      \
        if (detail::failed(var)) {                         \
            return;                                        \
        }                                                  \
        void(0)
#    define BOOST_HANDLEBARS_CHECK_OR_VALUE(var, value)              \
        if (detail::failed(var)) {                         \
            return value;                                  \
        }                                                  \
        void(0)
#    define BOOST_HANDLEBARS_CHECK_GET_OR_MACRO(_1, _2, NAME, ...) NAME
#    define BOOST_HANDLEBARS_CHECK_OR(...) \
        BOOST_HANDLEBARS_CHECK_GET_OR_MACRO(__VA_ARGS__, BOOST_HANDLEBARS_CHECK_OR_VALUE, BOOST_HANDLEBARS_CHECK_OR_VOID)(__VA_ARGS__)

#    define BOOST_HANDLEBARS_CHECK_OR_CONTINUE(var)                  \
        if (detail::failed(var)) {                         \
            continue;                                      \
        }                                                  \
        void(0)
...
public:
    ...
    {
        BOOST_HANDLEBARS_ASSERT(has_value_);
