...
setup_fixtures()
{
    master.template_path =
        BOOST_HANDLEBARS_TEST_FILES_DIR "/features_test.adoc.hbs";
    master.partial_paths = {
        BOOST_HANDLEBARS_TEST_FILES_DIR "/record-detail.adoc.hbs",
        BOOST_HANDLEBARS_TEST_FILES_DIR "/record.adoc.hbs",
        BOOST_HANDLEBARS_TEST_FILES_DIR "/escaped.adoc.hbs"};
    master.output_path =
        BOOST_HANDLEBARS_TEST_FILES_DIR "/features_test.adoc";
    master.error_output_path =
        BOOST_HANDLEBARS_TEST_FILES_DIR "/features_test_error.adoc";
    master.logger_output_path =
        BOOST_HANDLEBARS_TEST_FILES_DIR "/logger_output.txt";
    master.logger_error_output_path =
        BOOST_HANDLEBARS_TEST_FILES_DIR "/logger_output_error.txt";
...
setup_partials()
{
    ...
    for (auto partial_path: master.partial_paths)
    {
        ...
        std::string filename = files::getFileName(partial_path);
...
static
dom::Value
to_dom(boost::json::value& val)
{
    dom::Value res;
    if (val.is_object())
    {
        dom::Object obj;
        boost::json::object & json_obj = val.as_object();
        auto it = json_obj.begin();
        while (it != json_obj.end())
        {
            obj.set(it->key(), to_dom(it->value()));
            ++it;
        }
        res = obj;
        return res;
    ...
    if (val.is_double()) {
        std::string double_str = std::to_string(val.as_double());
...
mustache_compat_spec()
{
    ...
    std::string_view mustache_specs_dir =
        BOOST_HANDLEBARS_TEST_FILES_DIR "/mustache/";
    ...
    for (auto const& spec_file: spec_files) {
        ...
        boost::json::value json_value = boost::json::parse(content);
        //BOOST_TEST(jsonObj);
        //llvm::json::Value jsonData = std::move(*jsonObj);
        //BOOST_TEST(json_value.as_object());
        boost::json::object data = std::move(json_value.as_object());

        // Iterate tests
        boost::json::array tests = std::move(data["tests"].as_array());
        for (auto & testRef: tests) {
            boost::json::object test = testRef.as_object();
            // Skip invalid partial tests
            auto test_name =
                test.at("name").as_string();
            if (
            ...
            // Get template
            std::string template_str =test.at("template").as_string().c_str();
            ...
            if (test.contains("partials"))
            {
                boost::json::object partialsObj = test.at("partials").as_object();
                auto it = partialsObj.begin();
                bool incompatiblePartial = false;
                while (it != partialsObj.end())
                {
                    boost::json::string partial_string = it->value().as_string();
                    if (partial_string.find("{{=") != boost::json::string::npos)
                    {
                        // "{{=" not supported by handlebars
                    ...
                    {
                        partials.emplace_back(it->key(), partial_string);
                    }
                    ++it;
                }
            ...
            dom::Value context = to_dom(test.at("data"));
            HandlebarsOptions opt;
            opt.compat = true;
            std::string expected = test.at("expected").as_string().c_str();
